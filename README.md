...
- 股票代號與股票名稱搜尋。
- 透過 URL query 記錄股票與時間區間，可分享指定查詢狀態。
- 顯示目前股票名稱與代號。
...
- 將股票與時間區間同步到 `/?stock=2330&start_at=1609430400000&end_at=1778428799999`，讓查詢狀態可被分享與重新開啟。
...
首頁查詢狀態透過 URL query 保存：

- `stock`: 股票代號，例如 `2330`。
- `start_at`: 查詢起始時間，格式為 millisecond timestamp。
- `end_at`: 查詢結束時間，格式為 millisecond timestamp。

前端內部與 URL query 皆以 `Date` 的 timestamp 表示時間區間，只有在呼叫 FinMind API 前才轉成 `yyyy-MM-dd`。
...
app/hooks/                  # 前端狀態與瀏覽器能力封裝
  useRevenueSearchParams.ts # 將股票與時間區間同步到 URL query
...
- 股票與時間區間狀態放在 URL query，優先支援分享與重新開啟。
